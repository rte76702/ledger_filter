...
def validate_filters(filters, account_details):
	...
	if (filters.from_time > filters.to_time) and not (filters.to_date > filters.from_date):
...
def get_gl_entries(filters):
	...
	gl_entries = frappe.db.sql("""
		select
			posting_date, account, party_type, party, creation,
...
def get_accountwise_gle(filters, gl_entries, gle_map):
	...
	from_datetime = datetime.combine(getdate(filters.from_date), get_time(filters.from_time))
	to_datetime = datetime.combine(getdate(filters.to_date), get_time(filters.to_time))

	for gle in gl_entries:

		if (gle.creation < from_datetime) or gle.posting_date < from_date or cstr(gle.is_opening) == "Yes":
